...
def run(argv):
    ...
    if argv.NN == "knn":
        ...
        knnImgTest, knnLblTest, cnnImgTest, cnnLblTest, origin, pop20 = ut.OpenFile(argv.target, classes, argv.label, map)
        ...
        pareto80 = ut.OpenParetoFile(paretopath, pareto80file, totclasses)
        pareto20file = "pareto20.par"
        pareto20 = ut.OpenParetoFile(paretopath, pareto20file, totclasses)
        knnImgTrain, knnLblTrain, cnnImgTrain, cnnLblTrain, origin, pop80 = ut.OpenFiles(normpath, pareto80, totclasses, augment, map)
...
def main():
    ...
    if len(sys.argv) < 4:
        parser.add_argument('--nn', '-n', dest='NN', choices=['random', 'train', 'calibrate'], help="choose classification method")
    else:
