...
class CarDetector(object):
    ...
    def all_dnn(self, srcpath="path", dstpath = "path"):
        classes = None
        COLORS = None
        maxsize = 256
        minsize = 0
    ...
    def one_dnn(self, srcpath="path", dstpath="path"):
        classes = None
        COLORS = None
        maxsize = 256
        minsize = 0
        ...
        for i in indices:
            ...
            w = box[2]
            h = box[3]
            self.draw_prediction(image, class_ids[j], confidences[j], round(x), round(y), round(x + w),
                                 round(y + h), classes, COLORS)
            print("file: ", srcpath, " x:", x, "y:", y, " dimensions h:" + str(h) + " width: " + str(w))
            # cv2.imshow("object detection", image)
            ...
            normalised = cv2.equalizeHist(imgfile)
            cv2.imshow("gray", gray)
            root, filepath = srcpath.split('/')
            outputpath = os.path.join(dstpath, filepath)
            cv2.imwrite(outputpath, normalised)
